...
const nav: { to: string; label: string; icon: string; end?: boolean; pro?: boolean }[] = [
  ...
  { to: '/app/recompetes', label: 'Recompete Radar', pro: true, icon: 'M12 3a9 9 0 100 18 9 9 0 000-18zm0 4a5 5 0 100 10 5 5 0 000-10zm0 4a1 1 0 100 2 1 1 0 000-2zM12 3v2m9 7h-2M12 19v2M5 12H3' },
            ...
            <nav aria-label="Main" className="flex-1 space-y-1">
              {nav.map((item) => (
                ...
                >
                  ...
                  {item.label}
                  {item.pro && me?.plan !== 'Pro' && (
                    <span className="ml-auto rounded bg-indigo-100 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-indigo-700">Pro</span>
                  )}
                </NavLink>
              ))}
              <NavLink
